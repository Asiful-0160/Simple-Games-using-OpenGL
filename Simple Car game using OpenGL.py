...
class Game:
    ...
    def draw_explosions(self):
        ...
        for e in self.explosions:
            ...
            r = e["r0"] + (e["r1"] - e["r0"]) * t
            inten = 0.4 + 0.6*(1.0 - t)  # brightness fades (no alpha)

            glPushMatrix()
            glTranslatef(e["x"], e["y"], e["z"])
            glColor3f(1.0*inten, 0.6*inten, 0.1*inten)
            q = gluNewQuadric()
            gluSphere(q, r, 12, 12)

            # small bright core for a nicer punch
            glColor3f(1.0*inten, 1.0*inten, 0.4*inten)
            q2 = gluNewQuadric()
            gluSphere(q2, max(0.2, r * 0.35), 10, 10)
            glPopMatrix()
    ...
    def car(self):
        ...
        glColor3f(0.2, 0.6, 1.0) if self.shield_on else glColor3f(0.85, 0.05, 0.05)
        glPushMatrix()
        ...
        glColor3f(0.15, 0.5, 0.9) if self.shield_on else glColor3f(0.75, 0.03, 0.03)
        glPushMatrix()
        ...
        glColor3f(0.25, 0.65, 1.0) if self.shield_on else glColor3f(0.90, 0.10, 0.10)
        glPushMatrix()
    ...
    def draw(self):
        ...
        if self.state != GAME_PLAYING:
            ...
            if self.state == GAME_MENU:
                self.text(self.w/2 - 110, title_y, "3D DRIVING GAME", r=1.0, g=1.0, b=0.2)
                self.text(self.w/2 - 170, title_y - 40, "G : Start Game", r=1.0, g=1.0, b=1.0)
                self.text(self.w/2 - 170, title_y - 65, "Z : Controls", r=0.9, g=0.9, b=0.9)
                self.text(self.w/2 - 170, title_y - 90, "Q / ESC : Quit", r=0.9, g=0.9, b=0.9)
                self.text(self.w/2 - 260, title_y - 130, "Tip: Press W to toggle boost during gameplay", r=0.7, g=0.9, b=0.7)
                ...
                lines = [
                    ...
                    "F                : Activate NOVA (clear obstacles)",
                ...
                self.text(self.w/2 - 200, title_y - 75, "Right Click : Restart", r=0.9, g=0.9, b=0.9)
                self.text(self.w/2 - 200, title_y - 100, "B : Back to Menu", r=0.9, g=0.9, b=0.9)
            glPopMatrix()
            glMatrixMode(GL_PROJECTION)
            glPopMatrix()
            glMatrixMode(GL_MODELVIEW)
